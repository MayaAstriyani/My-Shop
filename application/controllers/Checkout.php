		...
		if ($this->input->post('submit', TRUE) == 'Submit')
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				if ($get->num_rows() > 0)
				{
					...
					$config['useragent'] 			= 'MyShop';
					$config['protocol'] 			= 'smtp';
					$config['mailtype'] 			= 'html';
					$config['smtp_host'] 			= 'ssl://smtp.gmail.com';
					$config['smtp_port']			= '465';
					$config['smtp_timeuot'] 	= '5';
					$config['smtp_user'] 			= 'mayshop100@gmail.com'; //isi dengan email gmail
					$config['smtp_pass'] 			= 'myshup101@'; //isi dengan password
					$config['crlf'] 					= "\r\n";
					$config['newline'] 				= "\r\n";
					...
					foreach ($this->cart->contents() as $carts) {
						$table .=
						'<tr><td>'.$no++.'</td><td>'.$carts['name'].'</td><td>'.$carts['qty'].'</td>
						<td style="text-align:right">'.number_format($carts['subtotal'], 0, ',', '.').'</td></tr>';
					}

					$this->email->from('mayshop100@gmail.com', "MyShop");
					$this->email->to($user->email);
					$this->email->subject('Pembayaran');
					$this->email->message(
						'Terimakasih telah melakukan pemesanan di toko kami, selanjutnya silahkan
						anda mentransfer uang senilai <b>Rp. '.number_format($total, 0, ',', '.').',-</b>
						ke no, rekening <b>90xxxx</b> paling lambat '.$bts.' agar pesanan anda bisa kami proses. Detail pembayaran sebagai berikut :<br><br>
	...
	public function city()
	{
		...
		curl_setopt_array($curl, array(
			...
			CURLOPT_HTTPHEADER => array(
				"key: 0542529ee76020b1b1e769d932503dbd"
			),
		));
		...
		if ($err) {
		  ...
		  for ($i=0; $i < count($data['rajaongkir']['results']); $i++) {
		      echo '<option value="'.$data['rajaongkir']['results'][$i]['city_id'].','.$data['rajaongkir']['results'][$i]['city_name'].'">
		...
		if ($err) {
			...
			for ($i=0; $i < count($data['rajaongkir']['results']); $i++) {
				...
				for ($l=0; $l < count($data['rajaongkir']['results'][$i]['costs']); $l++) {
					...
					echo '<option
					value="'.$data['rajaongkir']['results'][$i]['costs'][$l]['cost'][0]['value'].','
					.$data['rajaongkir']['results'][$i]['costs'][$l]['service'].'('.$data['rajaongkir']
					['results'][$i]['costs'][$l]['description'].')">';

					echo
					$data['rajaongkir']['results'][$i]['costs'][$l]['service'].'('.$data['rajaongkir']
					['results'][$i]['costs'][$l]['description'].')</option>';
				}
			}
		}
	}
