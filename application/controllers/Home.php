...
class Home extends CI_Controller {
    ...
    $data['data'] = $this->app->get_where('t_items', ['status' => 1]);
		$this->template->myshop('home', $data);
	}
	...
	public function detail()
	{
		if (is_numeric($this->uri->segment(3)))
		{
			...
			$this->template->myshop('item_detail', $data);
		} else {
			redirect('home');
		}
	}
	...
	public function registrasi()
	{
		if ($this->input->post('submit', TRUE) == 'Submit')
		{
			...
			if ($this->form_validation->run() == TRUE)
			{
				...
				{
					echo '<script type="text/javascript">alert("Akun Berhasil di buat,Silahkan Login !!");</script>';
					redirect('home/login');
				} else {
		...
		$this->template->myshop('register', $data);
	}
	...
	public function profil()
	{
			...
			if ($this->form_validation->run() == TRUE)
			{
				if (password_verify($this->input->post('pass', TRUE), $get->password))
				{
					...
					if ($this->app->update('t_users', $data, array('id_user' => $this->session->userdata('user_id'))))
					{
						...
						echo '<script type="text/javascript">alert("Profil berhasil diubah silahkan login ulang");window.location.replace("'.base_url().'/home/logout")</script>';
					}
				} else {
					echo '<script type="text/javascript">alert("Password
					Salah..");window.location.replace("'.base_url().'/home/logout")</script>';
			}
		}
	}
		...
		$this->template->myshop('user_profil', $data);
	}
	...
	public function password()
	{
		...
		if ($this->input->post('submit', TRUE) == 'Submit') {
			...
			if ($this->form_validation->run() == TRUE) {
				$get_data = $this->app->get_where('t_users', array('id_user' =>
				...
				if (!password_verify($this->input->post('pass3', TRUE), $get_data->password)) {
					...
					echo '<script type="text/javascript"> alert("Password berhasil dirubah silahkan login ulang");</script>';
					redirect('home/logout');
				}
			}
		}
		$this->template->myshop('pass');
	}
	...
	public function transaksi()
	{
		...
		$this->template->myshop('transaksi', $data);
	}
	...
	public function detail_transaksi()
	{
		...
		$this->template->myshop('detail_transaksi', $data);
	}
