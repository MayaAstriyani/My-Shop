          ...
          <tr>
            ...
            <td style="vertical-align:middle">
              <?php if ($batas < $today && $key->status == 'belum') { ?>
                <a href="<?= base_url(); ?>transaksi/delete/<?= $key->id_order; ?>" class="btn btn-danger" onclick="return confirm('Yakin Ingin Menghapus Data Ini ?')"><i class="f fa-trash"></i></a>
              <?php } else { ?>
                <a href="<?= base_url(); ?>transaksi/konfirmasi/<?= $key->id_order; ?>"
                  class="btn btn-success" <?php if ($key->status == 'proses') { echo 'disabled';} ?>><i class="fa fa-check"></i></a>
              <?php } ?>
